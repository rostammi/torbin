...
@section('content')
    <section class="container admin-page">
        ...
        <div class="sync-grid">
            <article class="panel sync-card"><span>{{ $stats['suggestions'] }} پیشنهاد آماده</span><h2>کشف تورهای محبوب</h2><p>کاتالوگ مقصدهای داخلی و خارجی را بخوان و فهرست پیشنهادها را به‌روز کن.</p><form method="post" action="{{ route('admin.sync.run') }}">@csrf<input type="hidden" name="type" value="discover_tours"><button class="button">اجرای کشف تورها</button></form></article>
        ...
        <div class="panel table-wrap">
            <table>
                ...
                @forelse ($runs as $run)
                    <tr>
                        <td><strong>{{ match($run->type) {'discover_tours' => 'کشف تورها', 'provision_tour' => 'ساخت خودکار تور', 'provision_all_tours' => 'ساخت/به‌روزرسانی همه تورها', 'prices' => 'قیمت‌ها', 'content' => 'محتواها', 'images' => 'تصاویر تورها', 'refresh_tour_images' => 'تعویض تصاویر یک تور', default => 'همگام‌سازی کامل'} }}</strong><small>{{ $run->user?->name ?? 'زمان‌بندی سیستم' }}</small></td>
